...
namespace SteelmakingExpanded.Structures.CowperStove.BlockEntities;
...
public class BlockEntityCowperStove : BlockEntityMultiblockStructure
{
  ...
  public override void Initialize(ICoreAPI api)
  {
    ...
    if (api.Side == EnumAppSide.Server)
    {
      ...
      if (system.GetNetworkAt(Pos) == null)
        system.AddNode(api.World.BlockAccessor, Pos, "gas");
    }
  }
  ...
  private float TryConsumeGas(float amount)
  {
    ...
    if (netManager.GetNetworkAt(Pos) is not GasNetwork gasNet)
      return 0f;
  ...
  private GasNetworkState? GetGasState()
  {
    var netManager = Api.ModLoader.GetModSystem<BlockNetworkModSystem>();
    return (netManager.GetNetworkAt(Pos) as GasNetwork)?.State;
  }
